...
const int MAX_LENGTH = 100; // Longitud máxima de cada palabra
const char DELIMITER = ' '; // Carácter delimitador
const char DELIMITER2 = ','; // Carácter delimitador
...
public:
	...
	void escribirPagina(const std::string& direccionPagina, const std::string& direccionBloque) {
		...
		if (!inputFile) {
			std::cout << "Error al abrir el archivo de la página." << std::endl;
			return;
		}

		char c;
		while (bloqueFile.get(c)) {
			if (c != '\n') { // Omitir caracteres de salto de línea
				inputFile.put(c);
			}
			else {
				inputFile << '\n';
	...
	void buscarPagina() {
		// Implementa la lógica para buscar un bloque específico en el bloque actual
...
public:
	...
	bool bloqueLleno() {
		...
		int fileSize = file.tellg();  // Obtiene el tamaño actual del archivo
		file.close();
	...
	int getCapacidadOcupada() {
		...
		int fileSize = bloqueFile.tellg();  // Obtiene el tamaño actual del archivo
		bloqueFile.close();
		return fileSize;
	}
	std::string getDireccion() const {
...
public:
	...
	void generarPagina(Pagina* pagina, std::string numero) {
		...
		// Verificar si la página ya está en el vector
		auto it = std::find(paginas.begin(), paginas.end(), direccion_pag);
		if (it != paginas.end()) {
			// La página ya está en el vector, moverla al inicio
			std::rotate(paginas.begin(), it, it + 1);
		}
		else {
			// La página no está en el vector, eliminar el último elemento si se alcanzó la capacidad máxima
			if (paginas.size() >= capacidad) {
				eliminarPagina();
			}
			// Agregar la nueva página al inicio del vector
			paginas.insert(paginas.begin(), direccion_pag);
		}

		pagina->escribirPagina(direccion_pag, direccion_blo);  // Escribir el bloque en el archivo de la página
	...
	void eliminarPagina() {
		if (!paginas.empty()) {
			...
			// Eliminar el archivo de la página
	...
	/*void buscarPagina(int numeroPagina) {
		if (numeroPagina >= 1 && numeroPagina <= paginas.size()) {
			...
			bloque.buscarBloque();
			// Mover la página al final del vector (actualizar LRU)
			std::rotate(paginas.begin() + numeroPagina - 1, paginas.begin() + numeroPagina, paginas.end());
		}
		else {
			std::cout << "La página especificada no existe." << std::endl;
		}
	}*/
	...
	int ubicar_registro(int contador2,int& ubi) {
		...
		while (true) {
			...
			if (!archivo) {
				break;  // No hay más archivos para leer
			}
...
public:
	...
	bool escribirArchivo(std::ifstream& inputFile) {
		...
		while (bytesEscritos < capacidadBytes && std::getline(inputFile, linea)) {
			...
			posActual = inputFile.tellg(); // Guardar la posición después de leer la línea
			if (linea.empty()) {
				continue;  // Omitir líneas vacías
			}
			if (linea.length() > capacidadBytes - bytesEscritos) {
				inputFile.seekg(posAnterior); // Retroceder al principio de la línea anterior
...
public:
	...
	void almacenar_archivo(const std::string& archivoTxt) {
		...
		while (!inputFile.eof()) {
			...
			if (discoLleno) {
				std::cout << "El disco está lleno. No se pueden almacenar más datos." << std::endl;
				break;
			}
		}
	...
	void obtener_archivo_Bloque() {
		...
		while (contador < cantSectores) {
			...
			if (!inputFile) {
				...
				delete bloque; // Liberar memoria antes de salir de la función
				return;
			}
	...
	void imprimir_sector(int sector) {
		...
		while (contador < cantSectores) {
			if (contador == (sector - 1)) {
				...
				int fileSize = sectorFile.tellg();  // Obtener el tamaño actual del archivo
		...
		//std::cout << "No se encuentra el sector" << std::endl; // Si no se encuentra el sector, se devuelve una cadena vacía
	}
	...
	void exportar_tabla() {
		...
		if (esquema.is_open()) {
			...
			int number = 0;
			int index = 0;
			int currentNumber = 0; // Número actual que se está leyendo
			bool isNumber = false;
			int numbers[tam_num];
			...
			if (archivo.is_open()) {
				...
				while (archivo.get(dataLine, MAX_LINE_SIZE, '\n')) {
					archivo.ignore(MAX_LINE_SIZE, '\n'); // Descarta el carácter de nueva línea
					int numerador = 0;
					...
					while (dataLine[i] != '\0') {
						...
						if (!inQuotes && dataLine[i] == ',') {
							...
							size_t bytes = j; // En este ejemplo, el tamaño en bytes se basa en la longitud del valor
				...
				//std::cout << "Archivo convertido con éxito." << std::endl;
				return;
			}
		}
		else {
...
void menu() {
	...
	ManagerBuffer* buffer = new ManagerBuffer(4, sectoresPorPista, capacidadSectorBytes); // Crear buffer de páginas con capacidad para 4 páginas
	...
	while (opcion != 11) {
		...
		case 1:
			...
			std::cout << "Tamaño de sectores:" << std::endl;
			std::cin >> capacidadSectorBytes;
			std::cout << "Cantidad de sectores en un bloque:" << std::endl;
			std::cin >> sectoresPorBloque;

			if (myDisco != nullptr) {
				delete myDisco; // Liberar memoria si ya se había inicializado un disco anteriormente
			}
			myDisco = new Disco(nombreDisco, nPlatos, pistasPorSuperficie, sectoresPorPista, capacidadSectorBytes, sectoresPorBloque);
			buffer = new ManagerBuffer(4, sectoresPorPista, capacidadSectorBytes); // Crear buffer de páginas con capacidad para 4 páginas
